...
const SpeedTypingGame = () => {
  const paragraphs = [
    ...
    "An aunt is a bassoon from the right perspective. As far as we can estimate, some posit the melic myanmar to be less than kutcha.One cannot separate foods from blowzy bows.The scampish closet reveals itself as a sclerous llama to those who look.A hip is the skirt of a peak.Some hempy laundries are thought of simply as orchids.A gum is a trumpet from the right perspective.A freebie flight is a wrench of the mind.Someposit the croupy.",
     "The browser's JavaScript engine creates a special environment to handle the transformation and execution of the JavaScript code. This environment is known as the Execution Context. The Execution Context contains the code that's currently running, and everything that aids in its execution. During the Execution Context run-time, the specific code gets parsed by a parser, the variables and functions are stored in memory, executable byte-code gets generated, and the code gets executed.",
     "One of the main security risks of using eval() is that it can execute arbitrary code, including malicious code. If the input to eval() is not sanitized properly, it can allow attackers to inject their own code into your application. JavaScript supports both OOP and functional programming paradigms. While it has traditionally been considered an OOP language, it has increasingly been used for functional programming due to its support for higher-order functions, closures, and other features.",
  ];
